...
def export_pytorch_model(param, pth_path, pt_path):
    ...
    print(model_path)
    net = RetinaFaceDetModule(model_path=model_path, H=param['h'], W=param['w'], use_cpu=True, loading=True)
    net.eval()
...
def _normalize_retinaface(img_cv2,npu=False):
...
def preprocess_retinaface(img_cv2, dst_size=[1024,720], npu=False):
    """
    :param img_cv2: 
    :param img_size: [w,h]
    :param mlu: 
    ...
    img_data = _normalize_retinaface(resized_img, npu=npu)
...
if __name__ == '__main__':
    ...
    model_path = 'weights/face_det/'
    model_pth_full_name = 'weights/face_det/mobilenet0.25_Final.pth'
    ...
    model_pt_full_name = pj(model_path, 'retinaface_{}_{}_{:d}x{:d}_{}.pt'.format(dtype,model_date,
    ...
    if not args.fast:
        ...
        pydata ,img,_ = preprocess_retinaface(img,[args_w, args_h])
