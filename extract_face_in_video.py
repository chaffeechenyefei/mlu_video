...
from udftools.functions import save_obj,load_obj,face_format,timer,datetime_verify,filter_time
...
if __name__ == '__main__':
    ...
    ##img source
    parser.add_argument('--vid_path',default='')#This path is video folder with several vids inside.
    ...
    vid_lst = [ c for c in os.listdir(args.vid_path) if c.endswith(args.vid_ext) and c.startswith(args.vid_head)]
    ...
    for vid_file in vid_lst:
        ...
        print('#' * 20)
        vid_fullname = pj( args.vid_path, vid_file )
        #create fold for saving result.
        result_fd = vid_file.split(args.vid_ext)[0]
        if os.path.isdir(pj(args.save_path,result_fd)):
            pass
        else:
            os.mkdir(pj(args.save_path,result_fd))
        ...
        with torch.no_grad():
            ...
            if os.path.isfile(vid_fullname):
                ...
                while True:
                    ...
                    get time of image
                    """
                    if pcnt > vid_fps*60 and last_ocr_time is None:
                    ...
                    for ith,img_face in enumerate(img_faces):
                        ...
                        cv2.imwrite(pj(args.save_path, result_fd,imgname), img_face)
