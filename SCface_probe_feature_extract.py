...
def main(args):
    ...
    imglst = [ c for c in os.listdir(pj(datapath)) if c.endswith('.jpg') ]
    ...
    with torch.no_grad():
        for cnt,imgname in enumerate(imglst):
            #decode image name
            ximgname = decode_image_name(imgname)
            uuid = ximgname['uuid']
            cam_id = ximgname['cam_id']
            if cam_id >= 6:
                #skip nir camera
                continue
            ...
            if uuid in db_feature.keys():
                db_feature[uuid]['feat'].append(probe_feat)
                db_feature[uuid]['face'].append(max_face.astype(np.float32))
            else:
                db_feature[uuid] = {'feat':[probe_feat],
                                    'face':[max_face.astype(np.float32)]}
    ...
    for uuid in db_feature.keys():
        feats = db_feature[uuid]['feat']
        feats = np.concatenate(feats,axis=0)
        np.save(pj(savepath,'%s.npy'%uuid),feats)

        faces = db_feature[uuid]['face']
        faces = np.stack(faces,axis=0) #[k,h,w,c]
        faces = faces.mean(axis=0)
        faces = faces.astype(np.uint8)
        probe_feat = infer.execute(faces).reshape(1, -1)  # [B,F]
        np.save(pj(savepath,'%s_merged.npy'%uuid),probe_feat)
